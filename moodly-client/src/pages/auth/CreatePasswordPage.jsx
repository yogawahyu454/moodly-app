...
export default function CreatePasswordPage() {
    ...
    // Variabel disesuaikan agar cocok dengan backend
    ...
    useEffect(() => {
        if (!userId) {
            alert("Terjadi kesalahan. Silakan mulai registrasi dari awal.");
            navigate("/register");
        }
    }, [userId, navigate]);
    ...
    const handleSubmit = (e) => {
        ...
        const data = {
            user_id: userId,
            password: password,
            password_confirmation: password_confirmation, // Nama field sudah sesuai
        };

        axios
            .post("http://127.0.0.1:8000/api/user/set-password", data)
            .then((response) => {
                alert("Kata sandi berhasil dibuat! Silakan masuk.");
                navigate("/login");
            })
            .catch((err) => {
                if (err.response && err.response.status === 422) {
                    ...
                    setError(
                        "Data yang dimasukkan tidak valid. Periksa kembali pesan di bawah input."
                    );
                } else {
    ...
    return (
        <div className="relative min-h-screen w-full bg-[#FFF0F0] flex items-center justify-center font-sans">
            <div className="w-full max-w-md min-h-screen shadow-2xl flex flex-col bg-transparent">
